...
export const UpdateDirector = ({ uniqueId }: UpdateDirectorProps) => {
  ...
  const actorsQuery = useDirector({ uniqueId })
  ...
  const director = actorsQuery.data?.data

  console.log('movies', moviesAllQuery?.data?.data)
  ...
  const addMovie = (uniqueId: string) => {
    if (uniqueId) {
      const actor = moviesAllQuery.data?.find((movie: Movie) => movie.uniqueId === uniqueId)
      const actorExists = reactiveMovies.find((movie) => movie.uniqueId === uniqueId)
      if (actor && !actorExists) setReactiveMovies((prevActors) => [...prevActors, actor])
    }
