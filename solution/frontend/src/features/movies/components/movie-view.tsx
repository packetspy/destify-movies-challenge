...
import { ArrowLeft, Link as LinkUrl, MessageSquareMore } from 'lucide-react'
        ...
        <div className="ml-5">
          ...
          <p className="mt-4">
            ...
            {movie.directors.map((director, index) => (
              <li key={index} className="ml-5 text-gray-700 list-none">
                <Link to={paths.app.director.getHref(director.uniqueId)} className="flex">
          ...
          <p className="mt-4">
            ...
            {movie.ratings.map((rating, index) => (
              <li key={index} className="ml-5 text-gray-700 list-none flex">
                <MessageSquareMore className="size-4 mr-2" /> {rating.source} - {rating.value}
              </li>
            ))}
          </p>
