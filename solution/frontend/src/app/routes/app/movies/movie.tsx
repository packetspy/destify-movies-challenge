...
import { useMovie, getMovieQueryOptions } from '@/features/movies/api/get-movie'
...
export const movieLoader =
  ...
  async ({ params }: LoaderFunctionArgs) => {
    ...
    const promises = [
      queryClient.getQueryData(movieQuery.queryKey) ?? (await queryClient.fetchQuery(movieQuery))
    ] as const
...
export const MovieRoute = () => {
  ...
  return (
    <>
      <ContentLayout title={movie.data.title}>
        <MovieView movieUniqueId={movieUniqueId} />
        <div className="mt-8">
          <ErrorBoundary
            fallback={<div>Failed to load comments. Try to refresh the page.</div>}
          ></ErrorBoundary>
        </div>
      </ContentLayout>
    </>
